    ...
    <>
                ...
                {Object.entries(prices).map(([key, val]: [string, any]) => {
                  ...
                  return (
                    <tr key={key}>
                      ...
                      <td>
                        <span style={{
                          color: val.trend === 'up' ? 'var(--accent-emerald)' : val.trend === 'down' ? 'var(--accent-rose)' : 'var(--text-tertiary)',
                          fontWeight: 600,
                        }}>
      ...
      <div className="card animate-in" id="azure-services">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12, padding: '4px 0' }}>
          {[
            { name: 'Azure App Service', icon: 'dns', desc: 'Backend API (Python FastAPI)', detail: 'Southeast Asia' },
            { name: 'Azure OpenAI GPT-4o', icon: 'psychology', desc: 'AI Insight Generation', detail: 'East US' },
            { name: 'Azure SQL Database', icon: 'database', desc: 'Transaction & Anomaly Storage', detail: 'pangantrace-srv' },
            { name: 'Azure Event Hubs', icon: 'stream', desc: 'Real-time Transaction Stream', detail: 'panganhub' },
            { name: 'Custom ML Model', icon: 'model_training', desc: 'Anomaly Detection & Price Forecasting', detail: 'Hosted on App Service' },
            { name: 'Azure Monitor', icon: 'monitoring', desc: 'Application Insights & Logging', detail: 'Auto-configured' },
          ].map((svc) => (
